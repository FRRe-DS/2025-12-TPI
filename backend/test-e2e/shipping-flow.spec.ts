...
// CONFIGURACIÓN
const KEYCLOAK_URL = process.env.KEYCLOAK_URL || 'https://keycloak.mmalgor.com.ar';
const REALM = process.env.REALM || 'ds-2025-realm';
const CLIENT_ID = process.env.CLIENT_ID || 'grupo-12';
const CLIENT_SECRET = process.env.CLIENT_SECRET;
// Credenciales de prueba
const USERNAME = process.env.USERNAME || 'test-user';
const PASSWORD = process.env.PASSWORD || '12deboca';
...
describe('Logistics Internal Shipping Flow', () => {
    ...
    it('🚢 Should Get Transport Methods', async () => {
        try {
            ...
            // La respuesta es un array directo, no un objeto con propiedad transport_methods
            expect(Array.isArray(response.data)).toBe(true);
            expect(response.data.length).toBeGreaterThan(0);
        } catch (error: any) {
            throw new Error(`Get Transport Methods failed: ${error.message}`);
        }
    });
